...
const GlosariDSingle = ({ glosariBRes, glosariRel }) => {
  const glosariD = glosariBRes.map((x) => x.attributes)
  const [{ title, description, body }] = glosariD
  return (
    <Container>
      <div>
        <div>
...
export async function getServerSideProps({ params }) {
  ...
  const glosariRel = await fetchAPI(
    `${process.env.NEXT_PUBLIC_API_URL}/api/glosariums?populate=*&filters[letter][alphabet][$eqi]=d`
  )
  return {
