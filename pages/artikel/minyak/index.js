      ...
      <div className='overflow-x-hidden '>
        <div className='bg-black bg-[url(/images/banner-bg.png)]'>
          <nav className='bg-black'>
            <Breadcrumb3 title1='Artikel' slug='artikel' title2='Minyak' />
          </nav>
          <HeroPage title='Minyak' />
        </div>
...
export async function getStaticProps() {
  const articlesRes = await fetchAPI(
    `${process.env.NEXT_PUBLIC_API_URL}/api/articles?populate=*&sort[0]=updatedAt%3Adesc&filters[category][title][$eqi]=minyak`
  )
